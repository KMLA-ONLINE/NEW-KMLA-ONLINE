...
- `message_attachments` — blob은 Storage `message-files`(평문) 또는 `message-files-encrypted`(암호문). 한 메시지에 최대 `max_message_attachments()`개, 단 **2개 이상이면 전부 `kind='image'`** 여야 한다. `width`/`height`/`duration_ms`는 클라이언트가 측정해 보고한다
  - 이름도 두 컬럼이다: `file_name`(평문) / `file_name_ciphertext`(암호문). 왜 파일명까지 가리는지는 [docs/e2ee.md](../../e2ee.md). 어느 쪽이 채워지는지는 버킷이 정하고(`message_attachments_name_shape_check`), 버킷은 대화 타입이 정한다(`trg_enforce_message_attachment_shape`)
...
| `get_encrypted_message_bodies(conversation_id, before_id?, limit)` | 대화 멤버 (**1:1만**)       | X    | 1:1 검색용 대량 읽기. 복호화에 필요한 최소한(암호문 + 봉투)만, 한 번에 1000개까지. `get_chat_messages`는 lateral join이 무겁고 100개에서 끊겨 대화 전체를 훑는 데 못 쓴다. 받는 쪽은 `app/lib/crypto/message-search.ts` |
