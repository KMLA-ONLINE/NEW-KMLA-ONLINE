...
- `messages` — 단일 `conversation_id` 참조. `parent_id` 답글(1레벨), `edited_at`/soft delete 상태. 편집 여부는 `edited_at is not null` 로 파생 (별도 boolean 없음)
...
| `get_chat_messages(conversation_id, before_id?, limit)`         | 대화 멤버                          | X    | 메시지 keyset 페이지네이션 (sender/parent/첨부/반응/읽음 포함)                   |
...
| `trg_mark_message_edited`           | `messages`          | BEFORE UPDATE of `content`                                   | 내용 trim, 내용이 바뀌면 `edited_at=now()` 스탬프               |
...
- 그룹 대화 생성(`conversations` insert), 그룹 멤버 추가, 일반 메시지 작성, 메시지 수정, 읽음 포인터 전진, 메시지 반응 insert/update/delete는 direct table access + RLS + column grant로 처리한다.
