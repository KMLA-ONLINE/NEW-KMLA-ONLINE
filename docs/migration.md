...
아래 표기에서 `?`는 nullable, `PK`는 primary key, `UQ`는 unique, `→`는 FK다. 명시되지 않은 컬럼은 추가하지 않는다. generated column과 FK 삭제 동작은 section 13 FK 원칙을 함께 적용한다.
...
* gongang CHECK는 section 13을 정본으로 삼는다.
...
* text bounds (모순 해소 후 확정값):
  * profile name 1 ~ 50
  * anonymous_username nullable 1 ~ 15 (`[M1]` 참고)
  * profile description 최대 2,000
  * space/club description 최대 5,000
  * group chat room name 1 ~ 30 (`[M2]` 참고)
  * club name, round name 1 ~ 100
...
원본 명세의 `search_posts`는 `ILIKE '%query%'`를 사용하도록 기술되어 있다. 이 방식은 세 가지 문제를 가진다. 첫째, 결과에 순위가 없어 관련도가 낮은 결과가 섞인다. 둘째, 어두 매칭만 가능해 한국어 부분 문자열에 취약하다. 셋째, 데이터가 늘어날수록 인덱스를 타지 못해 느려진다.

이미 section 02에서 `pg_trgm`을 활성화하고, section 06에서 `posts.search_vector`를 generated column으로, section 12에서 GIN 인덱스를 생성하므로 추가 migration 없이 검색 RPC 구현만 바꿔 정확도를 높일 수 있다.

### search_vector 컬럼 정의 변경

section 06에서 명시한 대로 title과 content에 가중치를 분리하여 저장한다.
...
-- comments.search_vector / messages.search_vector
search_vector tsvector GENERATED ALWAYS AS (
  to_tsvector('simple', coalesce(content, ''))
) STORED
```

`'simple'` config를 사용하는 이유: 한국어는 영어 어간 추출 규칙(`'english'`)이 적용되지 않는다. `'simple'`은 소문자화만 수행하므로 한국어·영어 혼용 환경에서 가장 예측 가능하다.

### 검색 RPC 구현 전략

FTS(`tsvector @@ tsquery`)는 단어 단위 매칭과 순위를 제공하지만 부분 단어(예: "공지" → "공지사항")를 놓칠 수 있다. `pg_trgm`은 부분 문자열과 오타에 강하지만 순위가 없다. 두 방식을 OR로 합산하면 recall과 precision을 모두 얻을 수 있다.

```sql
-- search_posts 핵심 구조 (의사 코드)
WITH fts AS (
  SELECT
    p.id,
    ts_rank(p.search_vector, websearch_to_tsquery('simple', p_query)) AS fts_score,
    'fts' AS match_type
  FROM posts p
  WHERE p.search_vector @@ websearch_to_tsquery('simple', p_query)
    AND p.deleted_at IS NULL
    -- RLS-equivalent: caller는 해당 space의 accepted·비차단 멤버
),
trgm AS (
  SELECT
    p.id,
    greatest(
      ...
      similarity(p.content, p_query)
    ) AS trgm_score,
    'trgm' AS match_type
  FROM posts p
  WHERE (p.title % p_query OR p.content % p_query)
    AND p.deleted_at IS NULL
    -- 동일 접근 제어
),
combined AS (
  SELECT
    ...
    coalesce(fts.fts_score, 0) * 2.0 + coalesce(trgm.trgm_score, 0) AS rank,
    coalesce(fts.match_type, trgm.match_type) AS match_type
  FROM fts
...
SELECT
  p.id AS post_id,
  p.title,
  ts_headline('simple', p.content, websearch_to_tsquery('simple', p_query),
    'MaxWords=30, MinWords=10, MaxFragments=1') AS content_snippet,
  private.display_author_name(p.author_id, p.is_anonymous) AS author_name,
...
ORDER BY c.rank DESC, p.created_at DESC
LIMIT 50;
```

`search_messages`도 동일한 패턴을 적용한다. messages는 단일 `content` 컬럼이므로 trgm score는 `similarity(m.content, p_query)` 하나만 사용한다.

### 구현 세부 계약

* `websearch_to_tsquery('simple', p_query)`를 사용한다. 사용자 입력을 그대로 받아도 SQL injection 위험이 없고, 따옴표·`-` 같은 웹 검색 문법을 해석한다.
* `similarity()` 임계값은 `pg_trgm.similarity_threshold` 기본값(0.3)을 사용한다. 이 값은 운영 파라미터이므로 migration SQL에 고정하지 않는다.
* FTS score에 `* 2.0` 가중치를 주어 정확한 키워드 매칭을 우선한다. 이 계수는 운영 중 조정 가능하며 migration SQL에 하드코딩한다.
* `ts_headline()`으로 매칭 위치를 하이라이트한 `content_snippet`을 반환한다.
* `match_type` 컬럼을 반환하여 클라이언트가 FTS 결과와 trgm fallback 결과를 구분할 수 있게 한다.
* 반환 rows 상한은 50으로 고정한다. 페이지네이션이 필요하면 별도 명세로 추가한다.
* 두 CTE 모두 RLS-equivalent 접근 제어(accepted·비차단 membership, 활성 space)를 WHERE 조건으로 명시한다. `search_posts`는 `SECURITY INVOKER`이므로 RLS가 자동 적용되지 않는 CTE 내부에서도 membership 조건을 직접 기술한다.
