...
- text bounds:
  - profile name, anonymous_username 1 ~ 50
  - profile description 최대 2,000
  - space name, group chat room name, club name, round name 1 ~ 100
  - space/club description 최대 5,000
  - post title 1 ~ 200, post content 1 ~ 50,000, comment/message content 1 ~ 10,000
  - notification title 최대 200, body 최대 2,000
...
- `grant_user_permission(p_user_id bigint, p_permission_key text) → void`와 `revoke_user_permission(p_user_id bigint, p_permission_key text) → void`를 제공하고 `granted_by`를 caller로 기록한다.
- `upsert_permission(p_key text, p_name text, p_description text) → void`를 제공하고 registry 삭제는 하지 않는다.
- `upsert_reaction_type(p_id bigint, p_key text, p_name text, p_icon text, p_sort_order int4) → void`를 제공하고 registry 삭제는 하지 않는다.
...
- 최초 app admin bootstrap은 `bootstrap_first_app_admin(p_profile_id bigint) → void` service-role 전용 RPC로 수행한다. 대상은 accepted profile이어야 하고 app admin이 이미 존재하면 거부한다.
...
- 예외적으로 profile 본인 SELECT와 `submit_onboarding`, `set_anonymous_username`, `withdraw_profile`은 각 계약에 명시된 상태를 직접 검사한다.
