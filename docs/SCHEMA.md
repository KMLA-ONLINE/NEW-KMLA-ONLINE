...
- role/status/status*updated*\* 및 탈퇴 처리는 RPC 전용
...
content text [null, note: '텍스트 없는 첨부-only 메시지를 위해 nullable']
...
Note: '''
CHECK: parent_id <> id
CHECK: content IS NULL OR trim 후 1 ~ 10,000자
sender는 room의 멤버여야 함
parent_id는 같은 room의 최상위 메시지만 가리킬 수 있음
content가 있는 메시지는 query와 함께 lower 처리하고 모든 공백을 제거한 뒤 ILIKE 부분 문자열 검색

첨부-only 메시지:

- send_message()는 텍스트-only 메시지용이며 content NULL/blank를 거부
- 첨부-only 또는 텍스트+첨부 메시지는 message-files에 `room_id/auth.uid()/uuid` 경로로 먼저 업로드한 뒤 send_message_with_attachment()가 message와 attachment metadata를 원자 생성
- 기존 메시지에 첨부를 추가하는 경우 `room_id/message_id/auth.uid()/uuid` 경로와 finalize_message_attachment()를 사용
...
indexes {
(room_id, id) [name: 'idx_messages_active_room_id', note: 'DESC on id, WHERE deleted_at IS NULL']
...
// - 특정 채팅방 내 메시지 검색
// - content가 있는 메시지만 띄어쓰기 무시 ILIKE '%query%' 매칭
// - deleted_at IS NULL
//
// Messenger RPC:
// - create_group_chat_with_members(name text, member_ids bigint[]) -> room_id
// - send_message(room_id bigint, content text, parent_id bigint?) -> message_id
// - send_message_with_attachment(room_id bigint, storage_path text, file_name text, content_type text, size_bytes int8, parent_id bigint?, content text?, width int4?, height int4?) -> message_id
// - mark_chat_read(room_id bigint, last_read_message_id bigint) -> void
// - set_message_reaction(message_id bigint, reaction_type_id bigint?) -> void
// - list_chat_rooms() -> table
// - get_chat_messages(room_id bigint, before_id bigint?, limit int4) -> table
