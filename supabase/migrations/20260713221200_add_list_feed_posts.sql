...
CREATE OR REPLACE FUNCTION public.list_feed_posts(p_before_id bigint DEFAULT NULL::bigint, p_limit integer DEFAULT 20)
 RETURNS TABLE(post_id bigint, pub_id uuid, title text, content text, is_anonymous boolean, author jsonb, is_mine boolean, category jsonb, space jsonb, pinned_at timestamp with time zone, created_at timestamp with time zone, updated_at timestamp with time zone, comment_count bigint, reaction_count bigint, top_reactions jsonb, my_reaction_id bigint, attachments jsonb)
 LANGUAGE plpgsql
...
-- 새 함수는 Postgres 기본값으로 PUBLIC에 EXECUTE가 열린다(db diff는 grant를 안 뽑는다).
-- 회수하지 않으면 anon도 호출할 수 있다. list_space_posts/get_post_comments는 create or
-- replace라 기존 grant가 그대로 유지되므로 여기서 다시 줄 필요가 없다.
revoke execute on function public.list_feed_posts(bigint,int4) from public, anon, authenticated, service_role;
grant execute on function public.list_feed_posts(bigint,int4) to authenticated;
