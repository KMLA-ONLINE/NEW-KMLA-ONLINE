...
// soft delete 30일이 지난 글·댓글의 하드 삭제. 행이 무한히 쌓이는 것도 문제지만, 더 나쁜 건
...
export default {
  fetch: withSupabase({ auth: "secret" }, async (_req, ctx) => {
    ...
    summary.purgedPosts = content?.[0]?.purged_posts ?? 0
    summary.purgedComments = content?.[0]?.purged_comments ?? 0
