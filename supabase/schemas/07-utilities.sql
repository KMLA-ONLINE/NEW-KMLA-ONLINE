create type public.gongang_location as enum (
  'floor_b1',
  'floor_2',
  'floor_4',
  'floor_10'
...
create index idx_gongangs_owner
on public.gongangs (
  owner_id
);

create index idx_gongangs_location_time
on public.gongangs (
  location,
  day_of_week,
  start_minute
...
create index idx_song_requests_requester_requested_at
on public.song_requests (
  requester_id,
  requested_at
);

create index idx_song_requests_requested_at
on public.song_requests (
  requested_at
);
...
alter table public.gongangs
  add constraint gongangs_day_of_week_check
    check (day_of_week between 0 and 6),
  add constraint gongangs_start_minute_check
    check (start_minute between 0 and 1439),
  add constraint gongangs_end_minute_check
    check (end_minute between 1 and 1440),
  add constraint gongangs_time_order_check
    check (start_minute < end_minute),
  add constraint gongangs_validity_check
    check (valid_from <= valid_until),
  add constraint gongangs_no_overlap
    exclude using gist (
      location with =,
      day_of_week with =,
      time_range with &&,
...
alter table public.song_requests
  add constraint song_requests_url_check
    check (
...
create policy gongangs_insert
on public.gongangs
for insert
to authenticated
with check (
  owner_id = private.current_profile_id()
  and private.has_permission('gongang')
);

create policy gongangs_update
on public.gongangs
for update
to authenticated
using (
...
to authenticated
with check (
  requester_id = private.current_profile_id()
  and private.has_permission('karaoke')
);
...
grant usage, select
on sequence
  public.gongangs_id_seq,
  public.song_requests_id_seq
to authenticated;
...
on sequence
  public.gongangs_id_seq,
  public.song_requests_id_seq,
  public.utility_bookings_id_seq
to service_role;
